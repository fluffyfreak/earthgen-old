...
bool Application::InitWindow()
{
	...
	const int screen_width = (int)Clamp((double)(mode->width - (mode->width >> 2)), (double)screen_height, screen_height * 1.7777778);
	const int screen_x_offset = (mode->width - screen_width) >> 1;
	const int screen_y_offset = (mode->height - screen_height) >> 1;

	_primaryWindow = glfwCreateWindow(screen_width, screen_height, "GLSLPlanet", NULL, NULL);
	...
	glfwMakeContextCurrent(_primaryWindow);
	glfwSwapInterval(1);
	glfwSetWindowCloseCallback(_primaryWindow, WindowCloseFunc);
	...
	glfwSetWindowTitle(_primaryWindow, "EarthGen");
	//GLFWimage icons[1];
	//glfwSetWindowIcon(_primaryWindow, 1, &icons[0]);

	glfwSetWindowPos(_primaryWindow, screen_x_offset, screen_y_offset);

	// Ensure we can capture the escape key being pressed below
	glfwSetInputMode(_primaryWindow, GLFW_STICKY_KEYS, GL_TRUE);
	glfwSetCursorPos(_primaryWindow, screen_width / 2, screen_height / 2);
	...
	glfwSetScrollCallback(_primaryWindow, ScrollFunc);
...
int Application::Run()
{
	do
	{
		...
		int	screenWide = 0;
		int	screenHigh = 0;
		glfwGetWindowSize(_primaryWindow, &screenWide, &screenHigh);
		...
		// Swap buffers
		glfwSwapBuffers(_primaryWindow);
		checkGLError();

		glfwPollEvents();
		NKeyboard::UpdateKeyStates(_primaryWindow);
	} // Check if the ESC key was pressed or the window was closed
	while (glfwGetKey(_primaryWindow, GLFW_KEY_ESCAPE) != GLFW_PRESS && (false == s_bShouldClose));
